...
export default function ShipperDashboard() {
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => setSimulatedShipmentId(null), 8000);
  ...
  const handleApproveReroute = async (shipmentId: string) => {
    const shipment = displayShipments.find((item) => item.id === shipmentId);
    if (!shipment) return;
    ...
    try {
      ...
      toast.success("Reroute approved", {
        description: "Reroute approved — transporter notified via WhatsApp in their preferred language",
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-6 text-white">
      <DashboardHeader
        ...
        title="Premium AI operations for every active lane."
        rightContent={
          <>
            <LiveMonitoringPill />
            ...
            >
              ...
              {activeAlerts.length ? <span className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-red-500" /> : null}
            </button>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Active Shipments", value: String(activeShipments), trend: "+12% this week", icon: Package, tone: "blue" as const },
          { label: "High Risk Alerts", value: String(highRiskAlerts), trend: highRiskAlerts > 0 ? "Needs attention now" : "No escalations", icon: ShieldAlert, tone: "orange" as const, pulse: highRiskAlerts > 0 },
          { label: "Avg On-Time Rate", value: `${onTimeRate}%`, trend: "+4.8% recovery", icon: TrendingUp, tone: "green" as const },
          { label: "Disruptions Prevented", value: String(disruptionsPrevented), trend: "This month", icon: WandSparkles, tone: "purple" as const },
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
        <article id="map" className="rounded-[28px] border border-white/8 bg-[#141414] p-5 shadow-[0_22px_60px_-30px_rgba(0,0,0,0.72)] sm:p-6">
              ...
              activeAlerts.slice(0, 5).map((shipment) => (
                <div key={shipment.id} className="rounded-[24px] border border-white/8 bg-[#101010] p-4">
                  ...
                  <p className="mt-3 text-sm leading-7 text-white/66">
                    {shipment.backend.activeAlert?.message || shipment.backend.explanation?.why || shipment.backend.statusMessage}
                  </p>

                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => handleApproveReroute(shipment.id)}
      ...
      <section className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <article id="shipments" className="rounded-[28px] border border-white/8 bg-[#141414] p-5 shadow-[0_22px_60px_-30px_rgba(0,0,0,0.72)] sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link to="/add-shipment" className="inline-flex items-center gap-2 rounded-full bg-[#AAFF45] px-4 py-2 text-sm font-semibold text-black transition hover:bg-[#95f12f]">
                      ...
                      <tr key={shipment.id} className={index % 2 === 0 ? "bg-[#141414]" : "bg-[#101010]"}>
                        ...
                        <td className="px-4 py-4">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Link to={`/shipment/${shipment.id}`} className="rounded-full border border-white/10 bg-transparent px-3 py-1.5 text-xs text-white/68 transition hover:border-[#AAFF45]/30 hover:text-white">
                              Open
                            </Link>
                          </div>
                        </td>
        ...
        <article id="timeline">
          <div className="rounded-[28px] border border-white/8 bg-[#141414] p-5 shadow-[0_22px_60px_-30px_rgba(0,0,0,0.72)] sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#101010] px-4 py-2 text-sm text-white/66">
                <Bell className="h-4 w-4 text-[#AAFF45]" />
                22 languages
              </div>
            </div>
